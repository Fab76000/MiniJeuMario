...
public class Scene extends JPanel {
    ...
    private boolean partieGagnee(){     
        if(this.compteARebours.getCompteurTemps() > 0 && Scene.mario.isVivant() == true && this.score.getNbrePieces() == 10 
            && this.xPos > 4400){
            if(this.ok == true){
    ...
    private boolean partiePerdue(){
        if(Scene.mario.isVivant() == false || this.compteARebours.getCompteurTemps() <= 0){return true;}
        else{return false;}
    }
    ...
    public void paintComponent(Graphics g) { // Dessin de toutes les images visibles à l'écran (appel toutes les 3 ms environ)
        ...
        for(int i = 0; i < this.tabObjets.size(); i++){
            // mario
            if(Scene.mario.proche(this.tabObjets.get(i))){Scene.mario.contact(this.tabObjets.get(i));}
            // champignons
        ...
        for(int i = 0; i < this.tabPieces.size(); i++){
            if(Scene.mario.proche(this.tabPieces.get(i))){
                if(Scene.mario.contactPiece(this.tabPieces.get(i))){
        ...
        for(int i = 0; i < this.tabChamps.size(); i++){ 
            if(Scene.mario.proche(this.tabChamps.get(i)) && this.tabChamps.get(i).isVivant() == true){
                Scene.mario.contact(this.tabChamps.get(i));
                if(this.tabChamps.get(i).isVivant() == false){Audio.playSound("/audio/ecrasePersonnage.wav");}
            }
        }
        // tortues
        for(int i = 0; i < this.tabTortues.size(); i++){
            if(Scene.mario.proche(this.tabTortues.get(i)) && this.tabTortues.get(i).isVivant() == true){
                Scene.mario.contact(this.tabTortues.get(i));
        ...
        if(Scene.mario.isVivant() == true){
            if(Scene.mario.isSaut()){g2.drawImage(Scene.mario.saute(), Scene.mario.getX(), Scene.mario.getY(), null);}
            else{g2.drawImage(Scene.mario.marche("mario", 25), Scene.mario.getX(), Scene.mario.getY(), null);}
        }else{g2.drawImage(Scene.mario.meurt(), Scene.mario.getX(), Scene.mario.getY(), null);}
